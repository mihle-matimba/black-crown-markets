...
var TOTAL_STEPS=PANE_REVIEW;

var leveragePaneHtml=HAS_LEV_STEP?(''
+'<div class="bcm-wizard-pane" data-pane="'+PANE_LEV+'">'
...
+'<div class="bcm-wiz-lev-grid" id="bcmLeverageGrid"></div>'
...
+'</div>'
+leveragePaneHtml
+'<div class="bcm-wizard-pane" data-pane="'+PANE_DETAILS+'">'
...
+'</div>'
+'<div class="bcm-wizard-pane" data-pane="'+PANE_REVIEW+'">'
...
var leverageGrid=HAS_LEV_STEP?wizard.querySelector('#bcmLeverageGrid'):null;
...
if(HAS_LEV_STEP)renderLeverageCards();
}
card.addEventListener('click',pick);
card.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();pick();}});
});
...
+'<div class="bcm-wiz-review-row"><span>Leverage</span><strong>1:500 by default</strong></div>';
...
goToStep(HAS_LEV_STEP?PANE_LEV:PANE_DETAILS);
return;
}
...
var panelBody=panelDefault?panelDefault.querySelector('.panel-body'):null;
var panelFooter=panelDefault?panelDefault.querySelector('.panel-footer'):null;
if(!accountSelect||!methodSelect||!panelDefault||!submitBtn||!amountInput||!amountGroup||!fieldsToHide||!panelBody||!panelFooter)return;
...
var skaleCrypto=skaleMethods.filter(function(o){return wdMethodMeta(o).label==='Crypto';})[0];
var cryptoCarrier=skaleMethods.filter(function(o){return o.value==='WireTransfer';})[0]||skaleMethods[0];
var methodChoices=skaleMethods.map(function(o){var m=wdMethodMeta(o);return{key:o.value,post:o.value,label:m.label,icon:m.icon,crypto:m.label==='Crypto'};});
if(!skaleCrypto&&cryptoCarrier)methodChoices.push({key:WD_CRYPTO_KEY,post:cryptoCarrier.value,label:'Crypto',icon:WD_METHOD_ICONS.crypto,crypto:true});
var brandMethod=null;
function methodPane(value){return value?form.querySelector('[id="'+value+'"]'):null;}
function currentChoice(){var k=brandMethod;return methodChoices.filter(function(c){return c.key===k;})[0]||null;}
function applyMethod(key){
var choice=methodChoices.filter(function(c){return c.key===key;})[0];
if(!choice)return;
brandMethod=choice.key;
if(methodSelect.value!==choice.post){methodSelect.value=choice.post;fireChange(methodSelect);}
var ownPane=(choice.key===WD_CRYPTO_KEY)?null:methodPane(choice.post);
Array.prototype.forEach.call(methodSelect.options,function(o){
var pane=methodPane(o.value);
if(pane)pane.style.display=(ownPane&&pane===ownPane)?'block':'none';
});
}
...
methodRowWrap.innerHTML='<div class="bcm-wd-method-list">'+methodChoices.map(function(c){
return '<div class="bcm-wd-method-row bcm-wd-method-card" data-value="'+esc(c.key)+'" role="button" tabindex="0"><span class="bcm-wd-method-icon">'+c.icon+'</span><span class="bcm-wd-method-name">'+esc(c.label)+'</span></div>';
}).join('')+'</div>'+currencyBoxHTML;
}else{
var soleMeta=methodChoices.length?methodChoices[0]:{label:'Wire Transfer',icon:WD_METHOD_ICONS.bank};
...
function highlightMethod(key){Array.prototype.forEach.call(methodCards,function(c){c.classList.toggle('active',c.getAttribute('data-value')===key);});}
Array.prototype.forEach.call(methodCards,function(card){
function pickMethod(){var key=card.getAttribute('data-value');applyMethod(key);highlightMethod(key);updateNavLabels();}
...
var defaultChoice=methodChoices.filter(function(c){return c.key==='WireTransfer';})[0]||methodChoices[0];
if(defaultChoice){applyMethod(defaultChoice.key);highlightMethod(defaultChoice.key);}
...
var TOTAL_STEPS=3;
function isCryptoMethod(){var c=currentChoice();return !!(c&&c.crypto);}
...
panelBody.insertBefore(stepBadge,panelBody.firstChild);
var step1Heading=document.createElement('div');
step1Heading.id='bcmWDStep1Heading';
step1Heading.innerHTML='<h3 class="bcm-wiz-pane-title">Select Account</h3><p class="bcm-wiz-pane-subtitle">Choose the trading account you want to withdraw from.</p>';
accountGroup.parentNode.insertBefore(step1Heading,accountGroup);
var step1ErrorEl=document.createElement('p');
...
if(currentStep===2){
if(!brandMethod){step2ErrorEl.classList.add('bcm-wiz-error-visible');return;}
...
if(!titleEl&&/pending document verification/i.test(t.textContent.trim()))titleEl=t;
});
if(!titleEl)return;
var panel=titleEl.closest('.panel');
if(!panel||panel.dataset.bcmWdGate)return;
panel.dataset.bcmWdGate='1';
panel.classList.add('bcm-wd-gate');
var col=panel.closest('.col-md-6');
if(col)col.classList.add('bcm-wd-gate-col');
var link=panel.querySelector('a.btn,a[href*="upload-documents"]');
var href=link?link.getAttribute('href'):'https://trade.blackcrownmarkets.com/upload-documents';
var heading=panel.querySelector('.panel-heading');
if(heading&&!heading.querySelector('.bcm-wd-gate-icon')){
var badge=document.createElement('div');
badge.className='bcm-wd-gate-icon';
badge.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l7 3v5c0 4.4-3 8.4-7 10-4-1.6-7-5.6-7-10V6l7-3z"/><path d="M9 12l2 2 4-4"/></svg>';
heading.insertBefore(badge,heading.firstChild);
}
var body=panel.querySelector('.panel-body');
if(body){
body.innerHTML='<div class="bcm-wd-gate-note">'
+'<p>In order to make a withdrawal, please upload your proof of address and bank statement for approval.</p>'
+'<p>If you have, please allow 4-6 working hours for approval.</p>'
+'</div>'
+'<a href="'+String(href).replace(/"/g,'&quot;')+'" class="bcm-wd-gate-btn">'
...
+'<span>Upload documents</span></a>';
}
}
function buildBankDetailsPage(){
var form=document.querySelector('form[action="https://trade.blackcrownmarkets.com/bank-details"]');
if(!form||form.dataset.bcmBankDetails)return;
var panelDefault=form.closest('.panel.panel-default')||form.parentElement;
var methodSelect=form.querySelector('#create_bank_details_withdrawal_method');
if(!panelDefault||!methodSelect)return;
form.dataset.bcmBankDetails='1';
panelDefault.classList.add('bcm-bank-details-panel');
var methodGroup=methodSelect.closest('.form-group');
methodSelect.style.display='none';
...
if(document.body.dataset.bcmAccountsPage)return;
document.body.dataset.bcmAccountsPage='1';
...
fields.accountSelect.dataset.bcmLevTypeNoted='1';
var typeNote=document.createElement('p');
typeNote.className='bcm-lev-type-note';
...
var wrapA=document.createElement('div');wrapA.className='bcm-lev-field';
var wrapB=document.createElement('div');wrapB.className='bcm-lev-field';
wrapA.appendChild(children[0]);wrapA.appendChild(children[1]);
wrapB.appendChild(children[2]);wrapB.appendChild(children[3]);
...
var sharedPanel=sumsubContainer.closest('.panel.panel-default');
if(!sharedPanel)return;
var headings=Array.prototype.filter.call(sharedPanel.children,function(el){return el.classList.contains('panel-heading');});
if(headings.length<2)return;
sumsubContainer.dataset.bcmSeparated='1';
var secondHeading=headings[1];
var newPanel=document.createElement('div');
newPanel.className='panel panel-default';
newPanel.style.marginTop='24px';
var node=secondHeading;
while(node){
var next=node.nextSibling;
newPanel.appendChild(node);
node=next;
}
sharedPanel.parentNode.insertBefore(newPanel,sharedPanel.nextSibling);
}
function stripUploadDocumentLabels(){
var form=document.querySelector('form[action="https://trade.blackcrownmarkets.com/api/upload-documents"]');
if(!form||form.dataset.bcmUploadLabelsStripped)return;
form.dataset.bcmUploadLabelsStripped='1';
document.querySelectorAll('.panel-heading').forEach(function(heading){
if(heading.textContent.trim()==='Upload a Document')heading.remove();
});
form.querySelectorAll('.control-label').forEach(function(label){
...
var DEPOSIT_MIN_BY_TIER={ZAR:{high:1700,low:170},USD:{high:100,low:10}};
function depositAccountTier(option){
...
if(!currencySelect.value||currencySelect.value==='ZAR'){currencySelect.value='USD';fireChange(currencySelect);}
